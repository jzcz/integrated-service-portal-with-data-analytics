...
for ($i = 0; $i < 12; $i++) {
    ...
    $dateRequested = date("Y-m-d", strtotime("+" . rand(0, 30) . " days"));
    ...
    if ($status === "Pending" || $status === "Declined") {
        ...
        $pickUpDate = date("Y-m-d", strtotime($dateRequested . " + 5 days"));
    }

    // Get a unique student number
    $availableStudentNos = array_diff($studentNos, $usedStudentNos);
    if (empty($availableStudentNos)) {
        break;
    }
    $studentNo = $availableStudentNos[array_rand($availableStudentNos)];
    ...
    $requests[] = [
        ...
        "studentNo" => $studentNo,
        "status" => $status,
        "proofImage" => ""     /* for image proof inputs */ 
    ];
}
...
<body>
        ...
        <div class="appt-controls-bar d-flex gap-3 align-items-center">
            <div class="d-flex align-items-center gap-2">
                ...
                <select name="status" id="" class="form-select">
                    ...
                    <option value="">For Pickup</option>
            ...
            <table class="table-bordered rounded table text-center">
                <thead class="">
                    <tr class="">
                        ...
                        <th scope="col mb-2" class="border-start-0 border-end-0">Date Requested</th>
    ...
    <tr>
        ...
        <td class="border-start-0 border-end-0 text-light-emphasis"><?php echo $a['studentNo'] ?></td>
        <td class="border-start-0 border-end-0 text-light-emphasis">
            <?php echo date("M j, Y, D", strtotime($a['dateRequested'])) ?>
        </td>
        <td class="border-start-0 border-end-0 text-light-emphasis">
            <?php echo ($a['pickUpDate'] === '--.--') ? '--.--' : date("M j, Y, D", strtotime($a['pickUpDate'])) ?>
        </td>
        <td class="border-start-0 border-end-0 text-light-emphasis">
                        ...
                        <div class="modal fade" id="<?php echo 'appt-details-modal' . $i + 1?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-xl modal-dialog-scrollable">  <!-- modal-fullscreen -->
                                ...
                                <div class="modal-body">
                                    <div class="px-1 gap-2">
                                        ...
                                        <div class="row mb-2">
                                            <div class="col">
                                                ...
                                                <input type="date" name="" class="form-control form-control-sm" id="" value="<?php echo $a['pickUpDate']?>" readonly>
                                            </div>
                                            <div class="col">
                                    ...
                                    <?php if (!empty($a['proofImage'])): ?>
                                    <div style="border: 1px solid #ced4da; padding: 0.75rem; border-radius: 0.375rem; background-color: #fff; display: flex; align-items: center; gap: 0.5rem;">
                                 ...
                                 <p class="form-text mb-0"><?php echo htmlspecialchars(basename($a['proofImage'])); ?></p>
                              </div>
                                 <?php else: ?>
                             <p class="text-muted fst-italic">No proof uploaded.</p>
                             <?php endif; ?>
                            </div>
                        ...
                        <div class="modal fade" id="<?php echo 'edit-appt-modal' . $i + 1?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            <label class="mb-1" for="">Current Status of the Request:</label>
                                            <div class="status-input-wrapper">
                                                <i class="bi bi-circle-fill text-in status-circle <?php echo $statusClassName[$a['status']]?>"></i>
                                        ...
                                        <div class="mb-2">
                                            <label class="text-secondary mb-1" for="">Confirm or Decline pending request?</label>
                                            <div>
                                        ...
                                        <?php if($a['status'] === "Upcoming") { ?>
                                            <div>
                                                <div class="mb-3">
                                                    ...
                                                    <input type="date" name="" class="form-control form-control-sm" id="" value="<?php echo $a['pickUpDate']?>">
                                        ...
                                        <?php if($a['status'] === "Cancelled") { ?>
                                            <div>
                                                ...
                                                <label for="" class="fst-italic form-text">*Cancelled requests cannot be edited further.</label> 
                                            </div>
                                        <?php } ?>
